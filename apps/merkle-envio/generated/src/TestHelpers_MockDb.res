...
and entities = {
  ...
  @as("Activity") activity: entityStoreOperations<Types.activityEntity>,
  @as("Asset") asset: entityStoreOperations<Types.assetEntity>,
  @as("Campaign") campaign: entityStoreOperations<Types.campaignEntity>,
...
let rec makeWithInMemoryStore = (inMemoryStore: IO.InMemoryStore.t) => {
  ...
  let entities = {
    ...
    activity: {
      module(IO.InMemoryStore.Activity)->makeStoreOperator(
        ~inMemoryStore,
        ~makeMockDb=makeWithInMemoryStore,
        ~getStore=db => db.activity,
        ~getKey=({id}) => id,
      )
    },
    asset: {
    ...
    campaign: {
      module(IO.InMemoryStore.Campaign)->makeStoreOperator(
        ~inMemoryStore,
        ~makeMockDb=makeWithInMemoryStore,
        ~getStore=db => db.campaign,
        ~getKey=({id}) => id,
      )
    },
    factory: {
...
let executeMockDbLoadLayer = (
  ...
  let entityExecutors = [
    ...
    makeMockDbEntityExecuter(
      ...
      ~dbReadFn=mockDb.entities.activity.get,
      ~inMemStoreSetFn=IO.InMemoryStore.Activity.set,
    ...
    makeMockDbEntityExecuter(
      ...
      ~dbReadFn=mockDb.entities.campaign.get,
      ~inMemStoreSetFn=IO.InMemoryStore.Campaign.set,
...
let writeFromMemoryStore = (mockDb: t, ~inMemoryStore: IO.InMemoryStore.t) => {
  ...
  mockDb->executeRows(
    ~inMemoryStore,
    ~getStore=self => {self.activity},
    ~getRows=IO.InMemoryStore.Activity.values,
    ~setFunction=IO.InMemoryStore.Activity.set,
    ~getKey=entity => entity.id,
  )
  mockDb->executeRows(
    ~inMemoryStore,
  ...
  mockDb->executeRows(
    ~inMemoryStore,
    ~getStore=self => {self.campaign},
    ~getRows=IO.InMemoryStore.Campaign.values,
    ~setFunction=IO.InMemoryStore.Campaign.set,
    ~getKey=entity => entity.id,
  )
  mockDb->executeRows(
    ~inMemoryStore,
