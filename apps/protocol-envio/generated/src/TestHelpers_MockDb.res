...
@genType
and entities = {
  @as("Action") action: entityStoreOperations<Types.actionEntity>,
  @as("Asset") asset: entityStoreOperations<Types.assetEntity>,
  @as("Batch") batch: entityStoreOperations<Types.batchEntity>,
  @as("Batcher") batcher: entityStoreOperations<Types.batcherEntity>,
  @as("Contract") contract: entityStoreOperations<Types.contractEntity>,
  @as("Segment") segment: entityStoreOperations<Types.segmentEntity>,
  @as("Stream") stream: entityStoreOperations<Types.streamEntity>,
  @as("Watcher") watcher: entityStoreOperations<Types.watcherEntity>,
}
...
let rec makeWithInMemoryStore = (inMemoryStore: IO.InMemoryStore.t) => {
  ...
  let entities = {
    action: {
      module(IO.InMemoryStore.Action)->makeStoreOperator(
        ~inMemoryStore,
        ~makeMockDb=makeWithInMemoryStore,
        ~getStore=db => db.action,
        ~getKey=({id}) => id,
      )
    },
    asset: {
      module(IO.InMemoryStore.Asset)->makeStoreOperator(
        ~inMemoryStore,
        ~makeMockDb=makeWithInMemoryStore,
        ~getStore=db => db.asset,
        ~getKey=({id}) => id,
      )
    },
    batch: {
      module(IO.InMemoryStore.Batch)->makeStoreOperator(
        ~inMemoryStore,
        ~makeMockDb=makeWithInMemoryStore,
        ~getStore=db => db.batch,
        ~getKey=({id}) => id,
      )
    },
    batcher: {
      module(IO.InMemoryStore.Batcher)->makeStoreOperator(
        ~inMemoryStore,
        ~makeMockDb=makeWithInMemoryStore,
        ~getStore=db => db.batcher,
        ~getKey=({id}) => id,
      )
    },
    contract: {
      module(IO.InMemoryStore.Contract)->makeStoreOperator(
        ~inMemoryStore,
        ~makeMockDb=makeWithInMemoryStore,
        ~getStore=db => db.contract,
        ~getKey=({id}) => id,
      )
    },
    segment: {
      module(IO.InMemoryStore.Segment)->makeStoreOperator(
        ~inMemoryStore,
        ~makeMockDb=makeWithInMemoryStore,
        ~getStore=db => db.segment,
        ~getKey=({id}) => id,
      )
    },
    stream: {
      module(IO.InMemoryStore.Stream)->makeStoreOperator(
        ~inMemoryStore,
        ~makeMockDb=makeWithInMemoryStore,
        ~getStore=db => db.stream,
        ~getKey=({id}) => id,
      )
    },
    watcher: {
      module(IO.InMemoryStore.Watcher)->makeStoreOperator(
        ~inMemoryStore,
        ~makeMockDb=makeWithInMemoryStore,
        ~getStore=db => db.watcher,
        ~getKey=({id}) => id,
      )
    },
  }
...
let executeMockDbLoadLayer = (
  ...
  let entityExecutors = [
    makeMockDbEntityExecuter(
      ...
      ~dbReadFn=mockDb.entities.action.get,
      ~inMemStoreSetFn=IO.InMemoryStore.Action.set,
      ~store=inMemoryStore.action,
      ~getEntiyId=entity => entity.id,
    ),
    makeMockDbEntityExecuter(
      ~idsToLoad=loadLayer.assetIdsToLoad,
      ~dbReadFn=mockDb.entities.asset.get,
      ~inMemStoreSetFn=IO.InMemoryStore.Asset.set,
      ~store=inMemoryStore.asset,
      ~getEntiyId=entity => entity.id,
    ),
    makeMockDbEntityExecuter(
      ~idsToLoad=loadLayer.batchIdsToLoad,
      ~dbReadFn=mockDb.entities.batch.get,
      ~inMemStoreSetFn=IO.InMemoryStore.Batch.set,
      ~store=inMemoryStore.batch,
      ~getEntiyId=entity => entity.id,
    ),
    makeMockDbEntityExecuter(
      ~idsToLoad=loadLayer.batcherIdsToLoad,
      ~dbReadFn=mockDb.entities.batcher.get,
      ~inMemStoreSetFn=IO.InMemoryStore.Batcher.set,
      ~store=inMemoryStore.batcher,
      ~getEntiyId=entity => entity.id,
    ),
    makeMockDbEntityExecuter(
      ~idsToLoad=loadLayer.contractIdsToLoad,
      ~dbReadFn=mockDb.entities.contract.get,
      ~inMemStoreSetFn=IO.InMemoryStore.Contract.set,
      ~store=inMemoryStore.contract,
      ~getEntiyId=entity => entity.id,
    ),
    makeMockDbEntityExecuter(
      ~idsToLoad=loadLayer.segmentIdsToLoad,
      ~dbReadFn=mockDb.entities.segment.get,
      ~inMemStoreSetFn=IO.InMemoryStore.Segment.set,
      ~store=inMemoryStore.segment,
      ~getEntiyId=entity => entity.id,
    ),
    makeMockDbEntityExecuter(
      ~idsToLoad=loadLayer.streamIdsToLoad,
      ~dbReadFn=mockDb.entities.stream.get,
      ~inMemStoreSetFn=IO.InMemoryStore.Stream.set,
      ~store=inMemoryStore.stream,
      ~getEntiyId=entity => entity.id,
    ),
    makeMockDbEntityExecuter(
      ~idsToLoad=loadLayer.watcherIdsToLoad,
      ~dbReadFn=mockDb.entities.watcher.get,
      ~inMemStoreSetFn=IO.InMemoryStore.Watcher.set,
...
let writeFromMemoryStore = (mockDb: t, ~inMemoryStore: IO.InMemoryStore.t) => {
  ...
  mockDb->executeRows(
    ~inMemoryStore,
    ~getStore=self => {self.action},
    ~getRows=IO.InMemoryStore.Action.values,
    ~setFunction=IO.InMemoryStore.Action.set,
    ~getKey=entity => entity.id,
  )
  mockDb->executeRows(
    ~inMemoryStore,
    ~getStore=self => {self.asset},
    ~getRows=IO.InMemoryStore.Asset.values,
    ~setFunction=IO.InMemoryStore.Asset.set,
    ~getKey=entity => entity.id,
  )
  mockDb->executeRows(
    ~inMemoryStore,
    ~getStore=self => {self.batch},
    ~getRows=IO.InMemoryStore.Batch.values,
    ~setFunction=IO.InMemoryStore.Batch.set,
    ~getKey=entity => entity.id,
  )
  mockDb->executeRows(
    ~inMemoryStore,
    ~getStore=self => {self.batcher},
    ~getRows=IO.InMemoryStore.Batcher.values,
    ~setFunction=IO.InMemoryStore.Batcher.set,
    ~getKey=entity => entity.id,
  )
  mockDb->executeRows(
    ~inMemoryStore,
    ~getStore=self => {self.contract},
    ~getRows=IO.InMemoryStore.Contract.values,
    ~setFunction=IO.InMemoryStore.Contract.set,
    ~getKey=entity => entity.id,
  )
  mockDb->executeRows(
    ~inMemoryStore,
    ~getStore=self => {self.segment},
    ~getRows=IO.InMemoryStore.Segment.values,
    ~setFunction=IO.InMemoryStore.Segment.set,
    ~getKey=entity => entity.id,
  )
  mockDb->executeRows(
    ~inMemoryStore,
    ~getStore=self => {self.stream},
    ~getRows=IO.InMemoryStore.Stream.values,
    ~setFunction=IO.InMemoryStore.Stream.set,
    ~getKey=entity => entity.id,
  )
  mockDb->executeRows(
    ~inMemoryStore,
    ~getStore=self => {self.watcher},
    ~getRows=IO.InMemoryStore.Watcher.values,
    ~setFunction=IO.InMemoryStore.Watcher.set,
    ~getKey=entity => entity.id,
  )
}
